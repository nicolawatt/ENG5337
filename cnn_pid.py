...
OFF_LINE_THRESHOLD = 0.6   

# PID gains — start here, tune in Session 2
Kp = 0.4
Ki = 0.01
Kd = 0.08
INTEGRAL_LIMIT = 1.0
...
def get_soft_error(frame_bgr):
    # Convert numpy array → PIL, force grayscale
    img = Image.fromarray(frame_bgr).convert('L')

    tensor = preprocess(img).unsqueeze(0).to(DEVICE)  # (1, 1, 128, 128)

    with torch.no_grad():
        logits = cnn_model(tensor)
        probs  = torch.softmax(logits, dim=1).squeeze().cpu().numpy()

    # Soft error: weighted sum of probabilities (off_line and straight = 0)
    ...
    return float(soft_err), off_line_prob, pred_class

#pid controller
class PIDController:
    """
    Discrete PID with anti-windup integral clamp.
    error > 0 (right of line) → positive output → increase left wheel
    error < 0 (left of line)  → negative output → increase right wheel
    """
...
#metrics
METRICS_CSV   = "cnn_pid_metrics.csv"
SUMMARY_FILE  = "cnn_pid_summary.txt"
...
csv_writer.writerow(['timestamp_s', 'pred_class', 'soft_err', 'cte_px',
                     'for_spd', 'turn_spd', 'delta_turn_spd',
        ...
        if probe.connected:
            ...
            if newHIL:
                ...
                if newDownCam:
                    ...
                    # ── CNN inference ──────────────────────────────────────────
                    soft_err, off_line_prob, pred_class = get_soft_error(gray_sm)
                    ...
                    if counterDown % 20 == 0:
                        print(f"{t:6.1f}  {pred_class:>9}  {soft_err:+8.3f}  "
                              f"{turnSpd:+7.3f}  {forSpd:7.3f}  {turnSpd:+8.3f}  "
                              f"{'YES' if off_line else 'no':>8}")
