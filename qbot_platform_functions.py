...
class QBPVision():
    ...
    def image_find_objects(self, image, connectivity, minArea, maxArea):
        ...
        (labels, ids, values, centroids) = cv2.connectedComponentsWithStats(image, connectivity)
        #---------------------------------------------------------------------#
    ...
    def line_to_speed_map(self, sampleRate, saturation):
        ...
        while True:
            ...
            if col is not None:
                ...
                #error: distance from image centre (pixel 160)
                error = 160 - col
                
                #angle from ref point at bottom centre (50, 160)
                angle = np.arctan2(error, 50)
                
                #PD controller
                turnSpd = kP * angle + kD * derivative.send(angle)
                
                #forward speed inversely proportional to error
